...
"""
FULL 3-STAGE HSOF GPU Pipeline Test for S4E10 Dataset
...
S4E10: Loan Default Prediction (58,645 samples, 26 features) - FULL DATASET NO SAMPLING
"""
...
println("FULL 3-STAGE HSOF GPU Pipeline Test - S4E10 Dataset (26 Features - FULL DATA)")
println("=" ^ 75)
...
# Load S4E10 full dataset with encoded features (58,645 samples)
println("\n📊 Loading S4E10 Full Dataset with Encoded Features (NO SAMPLING)...")
# Load pre-encoded data with all categorical features converted to numeric
df = CSV.read("competitions/playground-series-s4e10/s4e10_full_features_encoded.csv", DataFrame)
println("✅ Loaded encoded dataset: $(size(df)) with all features encoded")

# Get feature columns (exclude id and target)
feature_cols = [col for col in names(df) if col != "loan_status" && col != "id"]
println("✅ Features available: $(length(feature_cols)) features")
...
y = Float32.(df[!, target_col])  # loan_status - binary classification target (0/1)

# No normalization needed for binary classification target
...
# Select top features for Stage 2 - optimize for 26 features
stage1_keep = min(15, div(n_features, 2))  # Reduce 26→15 features in Stage 1
...
# MCTS parameters (optimized for 26 features dataset)
n_simulations = 100  # Balanced simulations for 58K samples
n_candidates = 60    # Good exploration for 15→8 feature reduction
...
# Train simple model on GPU for binary classification
println("Training GPU model for binary classification...")
stage3_start = time()

n_epochs = 100  # Sufficient epochs for binary classification
learning_rate = Float32(0.02)  # Slightly higher learning rate for binary problem
...
println("=" ^ 70)
println("Dataset: S4E10 ($(size(X, 1)) samples, 26 features)")
println("\nFeature Reduction:")
...
println("🎉 " * "="^75 * " 🎉")
println("                 HSOF GPU PIPELINE - WYNIKI KOŃCOWE S4E10")
println("🎉 " * "="^75 * " 🎉")
println()
...
println("   Problem:      Loan Default Prediction (Binary Classification)")
println("   Samples:      $(size(X, 1)) (FULL DATASET - NO SAMPLING)")
println("   Original:     $(length(feature_cols)) features (8 numeric + 18 encoded categorical) → $(stage3_keep) features")
println("   Target:       loan_status (0=no default, 1=default)")
println()
println("=" ^ 80)