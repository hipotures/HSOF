...
"""
FULL 3-STAGE HSOF GPU Pipeline Test for S4E12 Dataset
...
S4E12: Insurance Premium Prediction (1.2M samples, 155 features)
"""
...
println("FULL 3-STAGE HSOF GPU Pipeline Test - S4E12 Dataset")
println("=" ^ 75)
...
# Load S4E12 sample data (100K samples for faster demo)
println("\n📊 Loading S4E12 Sample Data...")
df = CSV.read("competitions/playground-series-s4e12/export/s4e12_train_sample.csv", DataFrame)
println("✅ Loaded: $(size(df)) (100K sample, preprocessed with encoded categoricals)")
...
stage1_keep = min(50, div(n_features, 3))  # Keep more features for large dataset (1.2M samples)
...
# MCTS parameters (increased for larger dataset)
n_simulations = 200  # More simulations for larger dataset
n_candidates = 100   # More candidates for better exploration
...
n_epochs = 200  # More epochs for larger dataset
...
println("   Samples:      $(size(X, 1)) (100K sample from 1.2M dataset)")
